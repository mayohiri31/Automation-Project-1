...
describe("This is first test suite, May Ohiri", () => {
  it("User can submit data only when valid mandatory values are added", () => {
    cy.get('[data-testid="phoneNumberTestId"]').type("0932999711");
    // April 24, 2024
    cy.get("#firstName").type("may");
    cy.get('[data-testid="lastNameTestId"]').type("ohiri");
    cy.get('input[name="password"]').type("Told123");
    cy.get('[name="confirm"]').type("Told123");
  ...
  it("User cannot submit data when phone number is absent", () => {
    ...
    // scroll back to the phone number input field
    ...
    // Check error message visibility: using CSS or simply be.visible
    // none = not visible; block = visible
    cy.get("#input_error_message").should("not.be.visible");
  });
  ...
  it.only("User cannot submit data when password is absent", () => {
    ...
    // scroll back to the pasword input field
